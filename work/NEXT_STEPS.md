...
| Track | Items | Why First |
|-------|-------|-----------|
| H1 | SonarQube + SpotBugs + Checkstyle + ESLint + Prettier setup | Quality gates before new code |
| H2 | OWASP Dependency-Check + Dependabot | Catch existing vulnerabilities |
| C2 | OpenAPI / Swagger docs | Document before testing |
| E2 | First 40 unit tests (service layer) | Foundation for coverage metrics |

### Phase 2 — Testing Ramp (Sprints 3–4)
| Track | Items | Why Now |
|-------|-------|--------|
| E2 | Remaining 130 unit tests | Build coverage to 70%+ |
| F2 | 80 new API tests (error paths, RBAC, validation) | Harden backend quality |
| G2 | Playwright setup + 30 P0/P1 E2E tests | Frontend confidence |
| A1 | 6 FE stories for R5 backend features | Ship visible value |

### Phase 3 — Infrastructure (Sprints 5–7)
| Track | Items | Why Now |
|-------|-------|--------|
| B2 | Redis caching | Quick performance win |
| C1 | Keycloak integration | Proper auth before scaling |
| C3 | Docker Compose improvements | Dev environment reliability |
| C4 | CI/CD pipeline | Automate all quality gates |

### Phase 4 — Advanced (Sprints 8–10)
| Track | Items | Why Last |
|-------|-------|---------|
| B1 | RabbitMQ event bus | Major architectural change |
| D1 | Reporting dashboards | New features on solid foundation |
| D2 | Scheduled jobs | Operational automation |
| G3 | Remaining 60 E2E tests | Full E2E coverage |
| H2 | OWASP ZAP DAST scanning | Test against deployed app |

### Phase 5 — Innovation (Sprints 11+)
| Track | Items |
|-------|-------|
| D3 | Enhanced AI (ML forecasting, dynamic pricing, NLP ordering) |
| D4 | Multi-tenant enhancements (onboarding wizard, branding, GDPR export) |
